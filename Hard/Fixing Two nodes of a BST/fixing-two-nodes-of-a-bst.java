...
class Solution
{
    ...
    public void answer(Node root)
    {
        if(root==null)
          return ;
          //inorder gives us bst in sorted order
        answer(root.left);  
        //here we have at max two adjustment 
        //so we create a prev first and second to manage thes adjusments
        if(prev!=null && root.data<prev.data)
         {
             
             if(first==null)//when first is null(only one time)
               first = prev;
              
              second = root;//(every time second will update)
         }
         prev= root;//when prev is null
         answer(root.right);
    }
    public Node correctBST(Node root)
    {
        //code here.
        answer(root);
        //we have to swap the values not the nodes as structure cant be violated
        int  temp = first.data;
